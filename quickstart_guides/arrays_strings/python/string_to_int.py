...
def string_to_int(str: str) -> int:
    sign = 0        # 0 positive: 1 negative
    value = 0
#        str = "2147483648" # debugging
    if(len(str) == 0):
        return(0)
    word = str.lstrip().split(' ')[0]
    if(word == ''):
        return(0)
    if((word[0] >= '0' and word[0] <= '9') or word[0] == '-' or word[0] == '+'):
        clean = word
    else:
        return(0)                       # ignore following characters
    ...
    for c in range(0,len(unsigned)):                # truncate non-number part
        if(unsigned[c] < '0' or unsigned[c] > '9'):
            unsigned = unsigned[0:c]
            break
        
    while unsigned != '':              # truncate leading zeros
        if(unsigned[0] == '0'):
            unsigned = unsigned[1:]
        else:
            break
