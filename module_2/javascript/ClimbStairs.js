 ...
 *
 *   Input:
 *      number n   => Total number of steps
 *   Output:
 *      number     -> Total number of unique ways to climb the stairs
 *
 *   Execution: node ClimbStairs.js
 */
...
var climbStairsBF = function(n) {
    ...
    var oneStep = climbStairsBF(n-1);

    // If we take 2 steps, how many combinations are there?
    var twoSteps = climbStairsBF(n-2);
...
var climbStairsTD = function(n) {
    ...
    return climbStairsTDInner(n, dp);
}
/**
 ...
 * @return{number}
 */
var climbStairsTDInner = function(n, dp) {
    // Base case. n is the number of steps remaining to the top. If n == 0,
    // then we are at the top of the stairs, so there is exactly 1 way to
    // get to the top of the stairs (aka do nothing)
    if (n == 0) return 1;
    ...
    if (dp[n] == 0) {
        ...
        dp[n] = climbStairsTDInner(n-1, dp) + climbStairsTDInner(n-2, dp);
    }
...
var tester = function() {
    console.assert(climbStairsBF(5) === 8);
    console.assert(climbStairsTD(5) === 8);
    console.assert(climbStairsBU(5) === 8);
